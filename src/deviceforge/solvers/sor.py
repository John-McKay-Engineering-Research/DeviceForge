...
class SORSolver(BaseSolver):
    """
    Two-dimensional Successive Over-Relaxation solver for the Laplace equation.

    SOR extends Gauss-Seidel iteration using a relaxation factor ``omega``:

        phi_new = (1 - omega) * phi_old + omega * phi_gauss_seidel
    ...
    def __init__(
        self,
        relaxation_factor: float = 1.8,
        configuration: SolverConfiguration | None = None,
    ) -> None:
        super().__init__(configuration)

        if not np.isfinite(relaxation_factor):
            raise ValueError("Relaxation factor must be finite.")

        if relaxation_factor <= 0.0 or relaxation_factor >= 2.0:
            raise ValueError(
                "Relaxation factor must be greater than 0 and less than 2."
            )

        self._relaxation_factor = float(relaxation_factor)
    ...
    def relaxation_factor(self) -> float:
        """Return the SOR relaxation factor."""
    ...
    def validate_simulation(
        ...
        if not np.isclose(
            ...
            raise ValueError(
                "SORSolver currently requires equal grid spacing "
        ...
        for boundary in simulation.neumann_boundaries:
            if not np.isclose(
                ...
                raise ValueError(
                    "SORSolver currently supports only homogeneous "
    ...
    def solve(
        ...
        """
        Solve the two-dimensional Laplace equation using SOR.
